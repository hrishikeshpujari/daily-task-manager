...
class TaskWidget : AppWidgetProvider() {
    ...
    companion object {
        ...
        // Theme palette + stickers now live in ThemePalette.kt, shared with MainActivity
        // (which uses it to match the WebView background/status bar to the current theme).
        ...
        fun build(context: Context, widgetId: Int = 0): RemoteViews {
            ...
            val pal = ThemePalette.paletteFor(themeId, Store.themeMode(context))
            val sticker = ThemePalette.randomSticker(themeId)
